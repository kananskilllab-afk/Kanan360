// Iscon Janmahaal (branch VAD05) 2nd floor -- 3600 sq.ft office & coaching
// floor, modelled from the architect's Drawing No. 01 (rev 07, 04-01-2025).
// True room geometry (walls, doors, glazing, furniture) rather than the
// generic per-area box the rest of the site uses (see FloorScene.tsx) --
// ported from a hand-authored reference build of this exact drawing.
// Internal coordinates are feet -> metres (`FT`); IsconJanmahaalFloor.tsx
// rescales the built group into the site's feet -> world-unit convention.
import * as THREE from 'three';

export const FEET_TO_METERS = 0.3048;
const FT = FEET_TO_METERS;
const f = (v: number) => v * FT;
...
// Premises outline, measured on a foot grid laid over the drawing: the
// plate steps in twice on the east/south and is notched on the west, where
// the lift, staircase and 2100mm passage are common building circulation.
const OUTLINE: Array<[number, number]> = [
  [0, 0], [75, 0], [75, 22], [72.5, 22], [72.5, 39], [62.5, 39],
...
function plate(name: string, mat: THREE.Material, depth: number, top: number) {
...
for (const k in M) M[k as keyof typeof M].name = k;

// ---------------------------------------------------------------- primitives
const box = (name: string, mat: THREE.Material, w: number, h: number, d: number, x: number, y: number, z: number, ry = 0) => {
...
const cyl = (name: string, mat: THREE.Material, r: number, h: number, x: number, y: number, z: number, seg = 20) => {
...
const openFloors: Array<{ id: string; r: [number, number, number, number] }> = [
...
function collectWalls(): WallEdge[] {
  const edges = new Map<string, WallEdge>();
  const add = (a: Point, b: Point, type: string, door: number | null) => {
    const k = key(a, b);
    let e = edges.get(k);
  ...
  for (const rm of rooms) {
    ...
    const sides: Record<string, [Point, Point]> = {
      n: [[x1, z1], [x2, z1]],
      s: [[x1, z2], [x2, z2]],
      w: [[x1, z1], [x1, z2]],
      e: [[x2, z1], [x2, z2]],
...
function wallSegments(e: WallEdge) {
...
function buildWalls(g: THREE.Group) {
...
function glazedWall(g: THREE.Group, name: string, ax: number, az: number, bx: number, bz: number, t: number) {
...
function solidWall(g: THREE.Group, name: string, ax: number, az: number, bx: number, bz: number, t: number) {
  const len = Math.hypot(bx - ax, bz - az);
  const ry = -Math.atan2(bz - az, bx - ax);
  g.add(box(`shell_${name}`, M.masonry, len, WALL_H, t, (ax + bx) / 2, 0, (az + bz) / 2, ry));
}

function buildShell(g: THREE.Group) {
  const t = SHELL;
  const i = t / 2;
...
// ---------------------------------------------------------------- furniture
function chair(g: THREE.Group, id: string, x: number, z: number, ry: number) {
  ...
  const bx = x - Math.sin(ry) * 0.8;
  const bz = z - Math.cos(ry) * -0.8;
  g.add(box(`${id}_back`, M.fabric, 1.5, 1.5, 0.25, bx, 1.6, bz, ry));
}

function desk(g: THREE.Group, id: string, x: number, z: number, w = 3.95, d = 1.95, ry = 0) {
  g.add(box(`${id}_top`, M.walnut, w, 0.14, d, x, 2.45, z, ry));
  const ox = Math.cos(ry) * (w / 2 - 0.3);
  const oz = -Math.sin(ry) * (w / 2 - 0.3);
  for (const s of [-1, 1]) g.add(box(`${id}_leg_${s > 0 ? 'r' : 'l'}`, M.metal, 0.2, 2.45, d - 0.4, x + s * ox, 0, z + s * oz, ry));
}

function credenza(g: THREE.Group, id: string, x: number, z: number, w: number, ry = 0) {
  g.add(box(`${id}_body`, M.walnut, w, 2.3, 1.25, x, 0, z, ry));
  g.add(box(`${id}_top`, M.ink, w + 0.15, 0.1, 1.4, x, 2.3, z, ry));
}

function screen(g: THREE.Group, id: string, x: number, z: number, w: number, ry = 0) {
  g.add(box(`${id}_panel`, M.ink, w, w * 0.56, 0.25, x, 3.6, z, ry));
}

function fit(g: THREE.Group, rm: RoomDef) {
  const [x1, z1, x2, z2] = rm.r;
  const w = x2 - x1;
  const d = z2 - z1;
  const mx = (x1 + x2) / 2;
  ...
  switch (rm.fit) {
    ...
    case 'reception': {
      ...
      const sofas: Array<[number, number]> = [[6.75, 4], [5.83, 3], [5.5, 3]];
      sofas.forEach(([len, seats], i) => {
        const zz = z2 - 2.4 - i * 4.6;
        const xx = x1 + w * 0.54;
    ...
    case 'lab': {
      ...
      for (const zz of rowsZ) {
        for (const side of [-1, 1]) {
          ...
          if (n >= (rm.seats ?? 0)) break;
          g.add(box(`${id}_bench_${n}`, M.walnut, 2.6, 0.14, 3.4, xx, 2.45, zz, 0));
...
// ---------------------------------------------------------------- core
function buildCommon(g: THREE.Group) {
  ...
  const treads = 13;
  const rise = 0.62;
  const going = 0.95; // 27 risers, two flights
...
// ---------------------------------------------------------------- labels
function labelSprite(title: string, sub: string, x: number, z: number, y: number, w = 13.5) {
...
export function buildFloor({ labels = true, furniture = true, glazing = true, useWash = true, common = true }: BuildFloorOptions = {}) {
  cx = PLAN_W / 2 - 4;
  cz = PLAN_H / 2;
...
export const commonList: Array<{ id: string; n: string; d: string; r: [number, number, number, number] }> = [
